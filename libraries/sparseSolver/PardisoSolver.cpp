...
PardisoSolver::PardisoSolver(const SparseMatrix * A, int numThreads_, int positiveDefinite_, int directIterative_, int verbose_) : numThreads(numThreads_), positiveDefinite(positiveDefinite_), directIterative(directIterative_), verbose(verbose_)
{
  ...
  // 0 = minimum degree ordering, 2=Fill-in reordering from METIS
  // 3 = The parallel(OpenMP) version of the nested dissection algorithm.It can decrease the time of computations on multi - core computers, 
  // especially when Intel MKL PARDISO Phase 1 takes significant time.
  iparm[1] = 3; 
  ...
  // See https://software.intel.com/en-us/node/470298
  // iparm[33] = numThreads_;
/*
  iparm[0] = 0;
  iparm[2] = numThreads;
*/
